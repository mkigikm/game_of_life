function CA (rows, cols, rule, torus) {
  this.rows  = rows;
  this.cols  = cols;
  this.rule  = rule;
  this.torus = torus;
...
CA.prototype.countInterior = function (row, j, top, bot) {
...
CA.prototype.countRightEdge = function (row, top, bot, firstState) {
  var count = new Int32Array(this.rule.states);
...
CA.prototype.countLeftEdge = function (row, top, bot) {
  var count = new Int32Array(this.rule.states);
...
CA.prototype.nextGeneration = function () {
  var first, top;
...
CA.prototype.birthRow = function (i, top, bot) {
  var row = this.grid[i];
  // save for torus
...
CA.prototype.birthCell = function (i, j, count) {
  this.setCellState(i, j, this.rule.transition(this.getCellState(i, j), count));
};

CA.prototype.getCellStateFromRow = function (row, j) {
  return (row[this.colShift(j)] >>> this.bitShift(j)) & this.mask;
}

CA.prototype.getCellState = function (i, j) {
  return this.getCellStateFromRow(this.grid[i], j);
};

CA.prototype.setCellState = function (i, j, state) {
...
CA.prototype.nextCellState = function (i, j) {
  var state = (this.getCellState(i, j) + 1) % this.rule.states;
  this.setCellState(i, j, state);
}

CA.prototype.colShift = function (j) {
  return ~~(j / this.perInt);
};

CA.prototype.bitShift = function (j) {
